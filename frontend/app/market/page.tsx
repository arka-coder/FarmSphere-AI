...
export default function MarketPage() {
 ...
 t ==="rising" ?"text-green-600" :
 t ==="falling" ?"text-red-600" :"text-amber-600";

 const trendBadge = (t: string) =>
 t ==="rising" ?"bg-green-50 text-green-700" :
 t ==="falling" ?"bg-red-50 text-red-700" :
"bg-amber-50 text-amber-700";
 ...
 {[
 { label:"Rising Crops", count: Object.values(markets).filter((m: any) => m.trend ==="rising").length, color:"text-green-600", bg:"bg-green-50"},
 { label:"Stable Crops", count: Object.values(markets).filter((m: any) => m.trend ==="stable").length, color:"text-amber-600", bg:"bg-amber-50"},
 { label:"Falling Crops", count: Object.values(markets).filter((m: any) => m.trend ==="falling").length, color:"text-red-600", bg:"bg-red-50"},
 ].map((s, i) => (
 ...
 className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-gray-200 bg-white text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-emerald-400"
 />
 </div>
 ...
 className="glass-card bg-white p-5"
 >
 ...
 <span className="font-semibold text-blue-600">₹{data.msp?.toLocaleString("en-IN")}/q</span>
 </div>
 )}
 <div className="flex items-center gap-1.5 text-xs text-gray-500">
 <MapPin size={11} />
 <span>Best: <strong className="text-gray-700">{data.best_market}</strong></span>
 </div>
 ...
 <p className="font-semibold text-gray-800 mb-3">📊 Market Insights</p>
