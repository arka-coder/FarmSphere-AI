...
const taskTypeConfig: Record<string, { icon: React.ReactNode; color: string; bg: string}> = {
 sowing: { icon: <Leaf size={14} />, color:"text-green-700", bg:"bg-green-50 border-green-100"},
 irrigation: { icon: <Droplets size={14} />, color:"text-blue-700", bg:"bg-blue-50 border-blue-100"},
 fertilizer: { icon: <FlaskConical size={14} />, color:"text-amber-700", bg:"bg-amber-50 border-amber-100"},
 harvest: { icon: <Scissors size={14} />, color:"text-purple-700",bg:"bg-purple-50 border-purple-100"},
 maintenance: { icon: <CheckCircle size={14} />, color:"text-slate-700", bg:"bg-slate-50 border-slate-100"},
 monitoring: { icon: <Clock size={14} />, color:"text-rose-700", bg:"bg-rose-50 border-rose-100"},
};
...
export default function CalendarPage() {
 ...
 className={`flex gap-4 p-4 rounded-xl border ${cfg.bg} ${t.is_due ?"ring-2 ring-emerald-400" :""} ${dimmed ?"opacity-50" :""}`}
 >
 <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${cfg.color} bg-white border border-current/20 shrink-0 mt-0.5`}>
 {cfg.icon}
 </div>
 <div className="flex-1 min-w-0">
 <div className="flex items-center justify-between gap-2 flex-wrap">
 <p className="font-semibold text-gray-800 text-sm">{t.task}</p>
 {t.is_due && (
 <span className="text-xs font-bold text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-full animate-pulse shrink-0">
 DUE NOW
 </span>
 )}
 </div>
 ...
 <h1 className="text-3xl font-bold tracking-tight capitalize drop-shadow-sm">{cropEmoji[crop] ||"🌱"} {crop.charAt(0).toUpperCase() + crop.slice(1)} Season Calendar</h1>
 <p className="text-emerald-200 text-sm mt-1">AI-generated task schedule based on ICAR guidelines</p>

 {/* Market price inline */}
 {marketInfo && (
 <div className="mt-3 flex items-center gap-3 bg-white/15 rounded-xl px-4 py-2 w-fit">
 <div>
 <p className="text-xs text-emerald-200">Current Market Price</p>
 <p className="text-2xl font-light tracking-tight">₹{marketInfo.price.toLocaleString("en-IN")}<span className="text-xs font-normal text-emerald-200">/quintal</span></p>
 </div>
 ...
 <span className="w-2 h-2 bg-emerald-500 rounded-full" /> Upcoming Tasks ({upcoming.length})
 </p>
