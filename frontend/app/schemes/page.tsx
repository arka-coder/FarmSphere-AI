...
export default function SchemesPage() {
 ...
 income_support:"bg-emerald-100 text-emerald-700",
 ...
 <p className="text-xs bg-amber-50 text-amber-700 border border-amber-100 rounded-lg px-3 py-2">
 ℹ️ {s.note}
 </p>
 </div>}
 </div>
 ...
 <span key={j} className="flex items-center gap-1 text-xs bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full">
 ...
 {s.online_apply_url ? (
 <><Globe size={12} className="text-green-500" /> <span>Apply online: <span className="font-medium text-gray-700">{s.apply}</span></span></>
 ) : (
 <><MapPin size={12} className="text-amber-500" /> <span>Apply at: <span className="font-medium text-gray-700">{s.apply}</span></span></>
 )}
 </div>
