      ...
      <main className="flex-1 overflow-y-auto p-8">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {stats.map((s, i) => (
              <motion.div
                ...
                className="glass-card bg-white dark:bg-gray-900 p-4 flex items-center gap-3"
              >
                ...
                <div>
                  ...
                  <p className="font-semibold text-gray-800 dark:text-white">{s.value}</p>
                </div>
              </motion.div>
            ))}
          </div>
          ...
          <div className="glass-card bg-white dark:bg-gray-900 p-5">
            <p className="font-semibold text-gray-800 dark:text-white mb-4">📅 5-Day Forecast</p>
            <div className="grid grid-cols-5 gap-3">
              {(w.forecast || []).slice(0, 5).map((f: any, i: number) => (
                <motion.div
                  ...
                  className="text-center bg-gray-50 dark:bg-gray-800 rounded-xl p-3"
                >
                  <p className="text-xs text-gray-500 dark:text-gray-400">{f.day}</p>
                  <p className="text-2xl my-1">{getEmoji(f.condition)}</p>
                  <p className="font-bold text-gray-800 dark:text-white text-sm">{f.temp_max}°</p>
          ...
          {/* Agricultural Advice */}
          <div className="glass-card bg-white dark:bg-gray-900 p-5">
            <p className="font-semibold text-gray-800 dark:text-white mb-3">🌾 Agricultural Weather Advice</p>
            <div className="space-y-2">
              {(w.forecast?.[0]?.rain_chance ?? 70) > 65 && (
                <div className="flex gap-3 p-3 bg-blue-50 dark:bg-blue-950/30 rounded-xl border border-blue-100 dark:border-blue-900">
                  <span>🌧️</span>
              ...
              {(w.humidity ?? 72) > 80 && (
                <div className="flex gap-3 p-3 bg-amber-50 dark:bg-amber-950/30 rounded-xl border border-amber-100 dark:border-amber-900">
                  <span>⚠️</span>
              ...
              <div className="flex gap-3 p-3 bg-green-50 dark:bg-green-950/30 rounded-xl border border-green-100 dark:border-green-900">
                <span>✅</span>
