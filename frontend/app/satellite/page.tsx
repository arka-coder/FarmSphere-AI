...
export default function SatellitePage() {
 ...
 h ==="Healthy" ?"text-green-600" :
 h?.includes("Moderate") ?"text-amber-600" :"text-red-600";

 const healthBg = (h: string) =>
 h ==="Healthy" ?"bg-green-50 border-green-200" :
 h?.includes("Moderate") ?"bg-amber-50 border-amber-200" :
"bg-red-50 border-red-200";
 ...
 <p className="font-semibold text-gray-800">📈 30-Day NDVI Timeseries</p>
 ...
 <div className="glass-card bg-white p-5">
 <p className="font-semibold text-gray-800 mb-3">🌾 Field Recommendations</p>
 <div className="space-y-2 text-sm">
 {data.vegetation_health ==="Healthy" ? (
 <div className="flex gap-2 p-3 bg-green-50 rounded-xl border border-green-100">
 <span>✅</span>
 <p className="text-green-700">Vegetation health is excellent. Continue current management practices. NDVI indicates optimal biomass accumulation.</p>
 </div>
 ) : (
 <>
 <div className="flex gap-2 p-3 bg-amber-50 rounded-xl border border-amber-100">
 <span>⚠️</span>
 <p className="text-amber-700">Vegetation stress detected. Check irrigation schedule and soil moisture. Consider foliar spray of micronutrients.</p>
 </div>
 <div className="flex gap-2 p-3 bg-blue-50 rounded-xl border border-blue-100">
 <span>💧</span>
 <p className="text-blue-700">Water stress index: {data.water_stress_index}. {data.water_stress_index > 0.5 ?"Irrigation required immediately." :"Monitor moisture levels closely."}</p>
 </div>
 </>
 )}
 </div>
